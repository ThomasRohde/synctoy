import { useEffect, useRef } from 'react';
import { useApp, useNotification, useDb } from '../context';
import { isValidUrl, getUrlPreview, getTextPreview, haptics } from '../utils';
import { encryptContent } from '../utils/crypto';
import type { PlainContent, EncryptedContent, DeviceCategory } from '../types';

/**
 * ShareReceiver - Component for receiving iOS Share Sheet deep links
 *
 * Parses and processes deep link URLs from iOS Shortcuts that enable one-tap
 * sharing into Synctoy Handoff from the iOS Share Sheet.
 *
 * Deep link format (query params on main URL, not hash):
 * ?handoff=1&nonce=...&kind=url|text&target=...&text=...&targetCategory=work|private|any
 *
 * This component processes the share link, adds the item to the database,
 * shows a toast notification, and cleans up the URL.
 */
export function ShareReceiver() {
    const { deviceProfile, refreshItems, navigate } = useApp();
    const notify = useNotification();
    const db = useDb();
    const processedRef = useRef(false);

    useEffect(() => {
        // Only process once per mount
        if (processedRef.current) return;

        const params = new URLSearchParams(window.location.search);

        // Check if this is a handoff share import link
        if (params.get('handoff') !== '1') {
            return; // Not a share link, no error needed
        }

        // Mark as processed to prevent re-processing on re-renders
        processedRef.current = true;

        // Parse and validate parameters
        const parsed = parseShareLink(params);

        if (!parsed) {
            // Invalid link
            notify.error('Nothing to send');
            cleanupURL();
            return;
        }

        // Check for duplicate via nonce
        if (isNonceDuplicate(parsed.nonce)) {
            // Silent ignore for duplicates
            cleanupURL();
            return;
        }

        // Store nonce to prevent future duplicates
        recordNonce(parsed.nonce);

        // Check work mode restrictions
        if (deviceProfile.workMode && parsed.kind === 'text') {
            notify.error('Work Mode is enabled. Only URLs are allowed.');
            cleanupURL();
            return;
        }

        // Process the share
        processShare(parsed);

        async function processShare(data: ParsedShareLink) {
            try {
                const plainContent: PlainContent = data.kind === 'url'
                    ? { url: data.target }
                    : { text: data.text };

                let itemContent: PlainContent | EncryptedContent = plainContent;

                // Encrypt if sensitive
                if (data.sensitive && data.passphrase) {
                    itemContent = await encryptContent(plainContent, data.passphrase);
                }

                // Generate preview
                const preview = data.kind === 'url'
                    ? getUrlPreview(data.target)
                    : getTextPreview(data.text);

                // Add item to database
                await db.addItem({
                    senderDeviceId: deviceProfile.deviceId,
                    senderDeviceName: deviceProfile.deviceName,
                    senderCategory: deviceProfile.category,
                    targetCategory: data.targetCategory || 'any',
                    kind: data.kind,
                    status: 'new',
                    isSensitive: data.sensitive || false,
                    content: itemContent,
                    preview,
                    title: data.title,
                });

                refreshItems();
                haptics.success();

                const source = data.source ? ` from ${data.source}` : '';
                notify.success(`Sent to Handoff${source}`);

                // Navigate to inbox to see the item
                if (data.openInbox) {
                    navigate('inbox');
                }
            } catch (error) {
                console.error('[ShareReceiver] Failed to send:', error);
                haptics.error();
                notify.error('Failed to send to Handoff');
            } finally {
                cleanupURL();
            }
        }
    }, [deviceProfile, db, notify, refreshItems, navigate]);

    // This component renders nothing
    return null;
}

/**
 * Interface for parsed share link parameters
 */
interface ParsedShareLink {
    nonce: string;
    kind: 'url' | 'text';
    target: string;
    text: string;
    targetCategory?: DeviceCategory;
    source?: string;
    title?: string;
    openInbox?: boolean;
    sensitive?: boolean;
    passphrase?: string;
}

/**
 * Parse and validate share link parameters
 */
function parseShareLink(params: URLSearchParams): ParsedShareLink | null {
    // Required: nonce (6-128 chars)
    const nonce = params.get('nonce');
    if (!nonce || nonce.length < 6 || nonce.length > 128) {
        console.warn('[ShareReceiver] Invalid nonce length:', nonce?.length);
        return null;
    }

    // Get kind (optional, can be inferred)
    let kind = params.get('kind') as 'url' | 'text' | null;
    const target = params.get('target') || '';
    const text = params.get('text') || '';

    // Infer kind if not provided
    if (!kind) {
        kind = target ? 'url' : 'text';
    }

    // Validate kind
    if (kind !== 'url' && kind !== 'text') {
        console.warn('[ShareReceiver] Invalid kind:', kind);
        return null;
    }

    // Validate for url kind
    if (kind === 'url') {
        if (!target) {
            console.warn('[ShareReceiver] Missing target for url kind');
            return null;
        }
        // Best-effort URL validation
        if (!isValidUrl(target)) {
            console.warn('[ShareReceiver] Invalid URL in target:', target);
            return null;
        }
        // Length check
        if (target.length > 4096) {
            console.warn('[ShareReceiver] Target URL too long:', target.length);
            return null;
        }
    }

    // Validate for text kind
    if (kind === 'text' && !text) {
        console.warn('[ShareReceiver] Missing text for text kind');
        return null;
    }

    // Length check for text
    if (text && text.length > 20000) {
        console.warn('[ShareReceiver] Text too long:', text.length);
        return null;
    }

    // Parse optional parameters
    const targetCategory = params.get('targetCategory') as DeviceCategory | null;
    const source = params.get('source') || undefined;
    const title = params.get('title') || undefined;
    const openInbox = params.get('open') === 'inbox';
    const sensitive = params.get('sensitive') === 'true';

    // Validate targetCategory if provided
    if (targetCategory && !['work', 'private', 'any'].includes(targetCategory)) {
        console.warn('[ShareReceiver] Invalid targetCategory:', targetCategory);
        return null;
    }

    return {
        nonce,
        kind,
        target,
        text,
        targetCategory: targetCategory || undefined,
        source,
        title,
        openInbox,
        sensitive,
    };
}

/**
 * Check if a nonce has been processed before
 *
 * Uses localStorage ring buffer to store last 50 nonces.
 */
function isNonceDuplicate(nonce: string): boolean {
    try {
        const stored = localStorage.getItem('synctoy_share_nonces');
        if (!stored) return false;

        const nonces = JSON.parse(stored) as string[];
        return nonces.includes(nonce);
    } catch (error) {
        console.error('[ShareReceiver] Error checking nonce:', error);
        // If we can't check, assume not duplicate to allow the share
        return false;
    }
}

/**
 * Record a nonce to prevent duplicate processing
 *
 * Maintains a ring buffer of 50 most recent nonces in localStorage.
 */
function recordNonce(nonce: string): void {
    try {
        const stored = localStorage.getItem('synctoy_share_nonces');
        let nonces: string[] = stored ? JSON.parse(stored) : [];

        // Add new nonce
        nonces.push(nonce);

        // Keep only last 50 (ring buffer)
        if (nonces.length > 50) {
            nonces = nonces.slice(-50);
        }

        localStorage.setItem('synctoy_share_nonces', JSON.stringify(nonces));
    } catch (error) {
        console.error('[ShareReceiver] Error recording nonce:', error);
        // Fail silently - we'll just risk a duplicate in this edge case
    }
}

/**
 * Clean up URL query parameters after processing
 *
 * Uses history.replaceState to remove share-related params without triggering navigation.
 */
function cleanupURL(): void {
    try {
        const url = new URL(window.location.href);
        // Remove all share-related params
        url.searchParams.delete('handoff');
        url.searchParams.delete('nonce');
        url.searchParams.delete('kind');
        url.searchParams.delete('target');
        url.searchParams.delete('text');
        url.searchParams.delete('targetCategory');
        url.searchParams.delete('source');
        url.searchParams.delete('title');
        url.searchParams.delete('open');
        url.searchParams.delete('sensitive');

        // Replace state without adding to history
        window.history.replaceState({}, '', url.toString());
    } catch (error) {
        console.error('[ShareReceiver] Error cleaning URL:', error);
        // Fail silently - not critical
    }
}
